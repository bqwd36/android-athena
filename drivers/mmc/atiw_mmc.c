...
struct atiw_mmc_host {
    void                    *ctl_base;
    struct device           *w228x_dev;       /* w228x device */
    struct atiw_mmc_hwconfig *hwconfig;       /* HW config data/handlers, guaranteed != NULL */
...
static void
mmc_finish_request(struct atiw_mmc_host *host)
{
...
#define ATIW_SD_CLK	80000000	/* HTC Athena, Hermes and Cheetah */
#define ASIC3_MMC_REG(host, block, reg) (*((volatile u16 *) ((host->ctl_base) + ((ATIW_SDIO_Base + ATIW_ ## block ## _ ## reg) >> (2 - host->bus_shift))) ))

static void
mmc_start_command(struct atiw_mmc_host *host, struct mmc_command *cmd)
{
    struct mmc_data *data = host->data;
    int c                 = cmd->opcode;

    DBG("Opcode: %d, base: %p reg_base: %p\n", cmd->opcode, host->ctl_base,(host->ctl_base + ((ATIW_SDIO_Base + ATIW_SDIO_CTRL_Cmd) >> (2 - host->bus_shift))) );

    if(cmd->opcode == MMC_STOP_TRANSMISSION) {
        ASIC3_MMC_REG(host, SDIO_CTRL, StopInternal) = SD_CTRL_STOP_INTERNAL_ISSSUE_CMD12;
    ...
    if(cmd->opcode == MMC_APP_CMD) {
        c |= SD_CTRL_COMMAND_TYPE_ACMD;
    }
    if (cmd->opcode == MMC_GO_IDLE_STATE) {
        c |= SD_CTRL_COMMAND_RESPONSE_TYPE_IDLE3;
    }
    if(data) {
        c |= SD_CTRL_COMMAND_DATA_PRESENT;
        if(data->blocks > 1) {
            ASIC3_MMC_REG(host, SDIO_CTRL, StopInternal) = SD_CTRL_STOP_INTERNAL_AUTO_ISSUE_CMD12;
    ...
    ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskCard) = ~(
    ...
    ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskBuffer) = ~(
    ...
    /* Send the command */
    ASIC3_MMC_REG(host, SDIO_CTRL, Arg1) = cmd->arg >> 16;
    ASIC3_MMC_REG(host, SDIO_CTRL, Arg0) = cmd->arg & 0xffff;
    ASIC3_MMC_REG(host, SDIO_CTRL, Cmd) = c;
}
...
{
    struct atiw_mmc_host *host = (struct atiw_mmc_host *)h;
    struct mmc_data *data = host->data;
    unsigned int *buf;
    int count;
    /* unsigned long flags; */
    ...
    buf += host->sg_ptr->offset/4 + host->sg_off/4;
    ...
    if(data->flags & MMC_DATA_READ) {
        while(count > 0) {
            ...
            *buf = ASIC3_MMC_REG(host, SDIO_CTRL, DataPort);
            buf++;
            count -= 4;
        }
	flush_dcache_page(host->sg_ptr->page);
        ...
        while(count > 0) {
            ...
            ASIC3_MMC_REG(host, SDIO_CTRL, DataPort) = *buf;
            buf++;
            count -= 4;
        }
    }
...
static void
mmc_data_end_irq(struct atiw_mmc_host *host)
{
    struct mmc_data *data = host->data;
    ...
    ASIC3_MMC_REG(host, SDIO_CTRL, StopInternal) = 0;

    /* Make sure read enable interrupt and write enable interrupt are disabled */
    if(data->flags & MMC_DATA_READ) {
        ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskBuffer) |= SD_CTRL_INTMASKBUFFER_BUFFER_READ_ENABLE;
    } else {
        ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskBuffer) |= SD_CTRL_INTMASKBUFFER_BUFFER_WRITE_ENABLE;
    }
...
static void
mmc_cmd_irq(struct atiw_mmc_host *host, unsigned int buffer_stat)
{
    ...
    if(cmd->flags & MMC_RSP_PRESENT && cmd->flags & MMC_RSP_136) {
        /* R2 */
        buf[12] = 0xff;
        data = ASIC3_MMC_REG(host, SDIO_CTRL, Response0);
        buf[13] = data & 0xff;
        buf[14] = data >> 8;
        data = ASIC3_MMC_REG(host, SDIO_CTRL, Response1);
        buf[15] = data & 0xff;
        buf[8] = data >> 8;
        data = ASIC3_MMC_REG(host, SDIO_CTRL, Response2);
        buf[9] = data & 0xff;
        buf[10] = data >> 8;
        data = ASIC3_MMC_REG(host, SDIO_CTRL, Response3);
        buf[11] = data & 0xff;
        buf[4] = data >> 8;
        data = ASIC3_MMC_REG(host, SDIO_CTRL, Response4);
        buf[5] = data & 0xff;
        buf[6] = data >> 8;
        data = ASIC3_MMC_REG(host, SDIO_CTRL, Response5);
        buf[7] = data & 0xff;
        buf[0] = data >> 8;
        data = ASIC3_MMC_REG(host, SDIO_CTRL, Response6);
        buf[1] = data & 0xff;
        buf[2] = data >> 8;
        data = ASIC3_MMC_REG(host, SDIO_CTRL, Response7);
        buf[3] = data & 0xff;
    ...
    if(buffer_stat & SD_CTRL_BUFFERSTATUS_CMD_TIMEOUT) {
        ...
        DBG("detail0 error status 0x%04x\n", ASIC3_MMC_REG(host, SDIO_CTRL, ErrorStatus0));
        DBG("detail1 error status 0x%04x\n", ASIC3_MMC_REG(host, SDIO_CTRL, ErrorStatus1));
...
//                    host->clock_for_sd = SD_CTRL_CARDCLOCKCONTROL_FOR_SD_CARD;
                    ASIC3_MMC_REG(host, SDIO_CTRL, CardOptionSetup) =
...
static void hwinit2_irqsafe(struct atiw_mmc_host *host);

static irqreturn_t
mmc_irq(int irq, void *irq_desc)
{
    struct atiw_mmc_host *host;
    unsigned int breg, bmask, bstatus, creg, cmask, cstatus;

    host = irq_desc;

    /* asic3 bstatus has errors */
    bstatus = ASIC3_MMC_REG(host, SDIO_CTRL, BufferCtrl);
    bmask   = ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskBuffer);
    cstatus = ASIC3_MMC_REG(host, SDIO_CTRL, CardStatus);
    cmask   = ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskCard);
    ...
    if (!breg && !creg) {
        ...
        ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskBuffer) &= ~breg;
        ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskCard) &= ~creg;
        goto out;
    }
    ...
    while (breg || creg) {
        ...
        if (creg & (SD_CTRL_CARDSTATUS_CARD_INSERTED_0 | SD_CTRL_CARDSTATUS_CARD_REMOVED_0)) {
            ASIC3_MMC_REG(host, SDIO_CTRL, CardStatus) &=
        ...
        if (breg & (SD_CTRL_BUFFERSTATUS_BUFFER_READ_ENABLE | SD_CTRL_BUFFERSTATUS_BUFFER_WRITE_ENABLE)) {
            ASIC3_MMC_REG(host, SDIO_CTRL, BufferCtrl) &=
        ...
        if (creg & SD_CTRL_CARDSTATUS_RW_END) {
            ASIC3_MMC_REG(host, SDIO_CTRL, CardStatus) &= ~(SD_CTRL_CARDSTATUS_RW_END);
...
out:
    ...
    ASIC3_MMC_REG(host, SDIO_CTRL, BufferCtrl) = 0;
    ASIC3_MMC_REG(host, SDIO_CTRL, CardStatus) = 0;
    return IRQ_HANDLED;
}

static void
mmc_start_data(struct atiw_mmc_host *host, struct mmc_data *data)
{
    ...
    ASIC3_MMC_REG(host, SDIO_CTRL, TransferBlockCount)	= data->blocks;
    ASIC3_MMC_REG(host, SDIO_CTRL, CardXferDataLen)	= data->blksz;
}
...
{
    struct atiw_mmc_host *host = mmc_priv(mmc);
...
/* Set MMC clock / power.
 * Note: This controller uses a simple divider scheme.
 */
static void
mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    DBG("requested clock %uHz busmode %u powermode %u Vdd %u\n",
        ios->clock, ios->bus_mode, ios->power_mode, ios->vdd);

    if (ios->clock) {
        clk = 0x80; /* slowest by default */
        if(ios->clock >= ATIW_SD_CLK / 256) clk >>= 1;
        if(ios->clock >= ATIW_SD_CLK / 128) clk >>= 1;
        if(ios->clock >= ATIW_SD_CLK / 64)  clk >>= 1;
        if(ios->clock >= ATIW_SD_CLK / 32)  clk >>= 1;
        if(ios->clock >= ATIW_SD_CLK / 16)  clk >>= 1;
        if(ios->clock >= ATIW_SD_CLK / 8)   clk >>= 1;
        if(ios->clock >= ATIW_SD_CLK / 4)   clk >>= 1;
        if(ios->clock >= ATIW_SD_CLK / 2)   clk >>= 1;
        if(ios->clock >= ATIW_SD_CLK / 1)   clk >>= 1;
...
//        }
        ...
        ASIC3_MMC_REG(host, SDIO_CTRL, CardClockCtrl) = 0;
        ASIC3_MMC_REG(host, SDIO_CTRL, CardClockCtrl) =
    ...
    switch (ios->power_mode) {
        case MMC_POWER_OFF:
		if (host->hwconfig && host->hwconfig->setpower)
			host->hwconfig->setpower(to_platform_device(mmc_dev(mmc)), ios->vdd);
            msleep(20);
            break;
        case MMC_POWER_UP:
            break;
...
mmc_get_ro(struct mmc_host *mmc)
{
    ...
    return (ASIC3_MMC_REG(host, SDIO_CTRL, CardStatus) & SD_CTRL_CARDSTATUS_WRITE_PROTECT)?0:1;
}
...
static void
hwinit2_irqsafe(struct atiw_mmc_host *host)
{
...
//    ASIC3_MMC_REG(host, SD_CONFIG, Command) = SD_CONFIG_COMMAND_MAE;
    ...
    ASIC3_MMC_REG(host, SDIO_CTRL, SoftwareReset) = 0; /* reset on */
    mdelay(2);

    ASIC3_MMC_REG(host, SDIO_CTRL, SoftwareReset) = 1; /* reset off */
    mdelay(2);
    ...
    ASIC3_MMC_REG(host, SDIO_CTRL, CardStatus) = 0;
    ASIC3_MMC_REG(host, SDIO_CTRL, BufferCtrl) = 0;
    ASIC3_MMC_REG(host, SDIO_CTRL, ErrorStatus0) = 0;
    ASIC3_MMC_REG(host, SDIO_CTRL, ErrorStatus1) = 0;
    ASIC3_MMC_REG(host, SDIO_CTRL, StopInternal) = 0;
...
//    ASIC3_MMC_REG(host, SD_CONFIG, ClockMode) = 0;
    ...
    ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskCard) = ~(
        ...
        ; /* check */
    ASIC3_MMC_REG(host, SDIO_CTRL, IntMaskBuffer) = 0xffff;  /* IRQs off */

    /*
     * ASIC3_MMC_REG(host, SDIO_CTRL, TransactionCtrl) = SD_CTRL_TRANSACTIONCONTROL_SET;
     *   Wince has 0x1000
     */
    /* ASIC3_MMC_REG(host, SDIO_CTRL, TransactionCtrl) = 0x1000; */
...
static void
hwinit(struct atiw_mmc_host *host, struct platform_device *pdev)
{
#if 0
...
{
    struct mmc_host *mmc;
    struct atiw_mmc_host *host = NULL;
    int retval = 0;
    struct atiw_mmc_hwconfig *mmc_config = (struct atiw_mmc_hwconfig *)pdev->dev.platform_data;
    ...
    mmc = mmc_alloc_host(sizeof(struct atiw_mmc_host) + 128, &pdev->dev);
    if (!mmc) {
        retval = -ENOMEM;
...
//    host->asic3_dev = pdev->dev.parent;
    ...
    if(!host->ctl_base){
	printk(KERN_ERR DRIVER_NAME ": Could not map ATIW SD controller\n");
        retval = -ENODEV;
        goto exceptional_return;
    }

    printk(DRIVER_NAME ": ATIW MMC/SD Driver, controller at 0x%lx\n", (unsigned long)pdev->resource[0].start);

    mmc->ops = &mmc_ops;
    mmc->caps = MMC_CAP_4_BIT_DATA;
    mmc->f_min = 312500; /* Hz, atihwtbl0.txt */
    mmc->f_max = 25000000; /* Hz, SD */
...
static struct platform_driver mmc_atiw_driver = {
    .driver = {
	.name    = DRIVER_NAME,
    },
    .probe   = mmc_probe,
...
{
    return platform_driver_register(&mmc_atiw_driver);
}

static void __exit mmc_exit(void)
{
    platform_driver_unregister(&mmc_atiw_driver);
}

late_initcall(mmc_init);
module_exit(mmc_exit);

MODULE_DESCRIPTION("ATIW SD/MMC driver");
