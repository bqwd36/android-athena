...
static inline unsigned long asic3_address(struct device *dev, unsigned int reg)
{
...
{
	printk(KERN_INFO "asic3: asic3_probe: STARTING!!!!!!!!!!!!!!!!!!!!!\n");
...
{
	struct asic3_data *asic = platform_get_drvdata(pdev);
	suspend_cdex = __asic3_read_register(asic, _IPAQ_ASIC3_CLOCK_Base + _IPAQ_ASIC3_CLOCK_CDEX);
	/* The LEDs are still active during suspend */
	__asic3_write_register(asic, _IPAQ_ASIC3_CLOCK_Base + _IPAQ_ASIC3_CLOCK_CDEX, suspend_cdex & ASIC3_SUSPEND_CDEX_MASK);
	return 0;
}
...
static int __init asic3_base_init(void)
{
	...
	if (retval) {
		printk(KERN_ERR "ASIC3 Base: Driver NOT registered successfully (Return code:%d).\n", retval);
	} else {
		printk(KERN_NOTICE "ASIC3 Base (%s): DEBUG: Driver registered successfully (Return code: %d).\n", __FILE__, retval);
	}
