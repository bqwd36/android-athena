...
static int __init egpio_init(void)
{
	...
	if (retval = platform_driver_register(&egpio_driver)) {
		printk(KERN_NOTICE "HTC-EGPIO: DEBUG: Driver registered successfully (Return code:%d).\n", retval);
	} else {
		printk(KERN_NOTICE "HTC-EGPIO: DEBUG: Driver NOT registered successfully (Return code:%d).\n", retval);
	}
