...
{
	printk(KERN_NOTICE "asic3_keys: DEBUG: asics_probe() called");
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		set_bit(b->keycode, pdata->input->keybit);
                ret = request_irq(base_irq + b->gpio, asic3_keys_asic_handle, IRQF_SAMPLE_RANDOM, b->desc, pdata);
                if (ret) {
...
static int __init asic3_keys_init(void)
{
	...
	r = platform_driver_register(&asic3_keys_driver);

	if (r) {
		printk(KERN_NOTICE "ASIC3 Key: Driver registered successfully (Return code:%d).\n",r);
	} else {
		printk(KERN_NOTICE "ASIC3 Key: Driver NOT registered successfully (Return code:%d).\n",r);
	}
